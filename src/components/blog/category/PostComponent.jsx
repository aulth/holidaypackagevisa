...
const PostComponent = ({data, category}) => {
    ...
    return (
        <div className="w-full flex flex-col md:flex-row sm:flex-row gap-2 justify-start md:h-44 h-auto  mt-2">
            <div className="w-full md:w-60 sm:w-60 shrink-0 h-44 relative">
                <button className="px-1.5 py-0.5 rounded-lg top-2 left-2 text-sm bg-gradient-to-r from-red-500 to-red-200 absolute text-white">
                    {category[0].toUpperCase()+category.slice(1).toLowerCase()}
                </button>
                <img src={data?.cover} className='w-full h-44 rounded-lg object-cover' alt="" />
            </div>
            <div className="w-full">
                <Link href={`/blog/${data.link}`} className="font-bold text-xl hover:text-red-500">{data?.title.slice(0,85)}{data?.title.length>85?"..":""}</Link>
                <p className='mt-1'>

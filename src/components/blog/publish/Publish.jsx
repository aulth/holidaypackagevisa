...
class MyComponent extends React.Component {
    ...
    handleOnSave = async (e) => {
        ...
        const response = await fetch('/api/blog/create', {
            ...
            body: JSON.stringify({ data: { title: this.state.title, category: this.state.category, cover: this.state.cover, content: this.state.content, live: false }, adminPin:process.env.NEXT_PUBLIC_ADMIN_PIN })
        })
    ...
    handleOnPublish = async (e) => {
        ...
        const response = await fetch('/api/blog/create', {
            ...
            body: JSON.stringify({ data: { title: this.state.title, category: this.state.category, cover: this.state.cover, content: this.state.content, live: true }, adminPin:process.env.NEXT_PUBLIC_ADMIN_PIN })
        })
        const json = await response.json();
        if (json.success) {
            toast.success(json.msg)
                ...
                <div className="container m-auto md:h-screen h-[calc(100vh-56px)] md:mt-0 mt-[56px] overflow-y-auto  relative">
                    <form onSubmit={this.handleOnPublish} className="w-full flex flex-col gap-4 bg-white rounded-lg p-4">
                        ...
                        <div className="flex items-center ">
                            <input type="text" name='title' onChange={this.handleChange} placeholder='Title...' className='p-2 border-b border-gray-200 w-full text-lg font-semibold focus:border-cyan-400 focus:outline-none' />
                        </div>
                        <div className="flex items-center ">
                            <input type="text" name='category' onChange={this.handleChange} placeholder='Category' className='p-2 border-b border-gray-200 w-full focus:border-cyan-400 focus:outline-none' />
                        </div>
